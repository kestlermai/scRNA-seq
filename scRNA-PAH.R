...
setwd("C:/Users/maihuanzhuo/Desktop/scRNA/scRNA-PAH")

sc.data <- Read10X(data.dir = "./raw")
sce.all <- CreateSeuratObject(counts = sc.data, project = "GSE233189", min.cells = 3, min.features = 200)

sample <- lapply(colnames(sce.all), function(x){
  strsplit(x,'-')[[1]][2]}) %>% 
  unlist() %>% 
  as.character()

sample <- ifelse(as.numeric(sample) <= 6, paste0("PAH", sample), paste0("CON", sample))

unique(sample)
names(sample) <- colnames(sce.all)
sce.all[['sample']] <- sample
unique(sce.all$sample)

sce.all$sample <- factor(sce.all$sample, levels = c("PAH1","PAH2","PAH3","PAH4","PAH5","PAH6",
...
p1 <- VlnPlot(sce.all, group.by = 'sample', features = feats, pt.size = 0, ncol = 2) + 
  NoLegend()
ggsave(filename="Vlnplot1.pdf", p1, width = length(unique(sce.all$sample))/3+5, height = 5, dpi = 600)
feats <- c("percent_mito", "percent_ribo", "percent_hb")
p2 <- VlnPlot(sce.all, group.by = 'sample', features = feats, pt.size = 0, ncol = 3, same.y.lims = T) + 
  scale_y_continuous(breaks=seq(0, 100, 5)) +
  NoLegend()
ggsave(filename="Vlnplot2.pdf", p2, width = length(unique(sce.all$sample))/3+5, height = 5, dpi = 600)

p3.feature <- FeatureScatter(sce.all, "nCount_RNA", "nFeature_RNA", group.by = 'sample', pt.size = 0.5)
p3.mito <- FeatureScatter(sce.all, "nCount_RNA", "percent_mito", group.by = 'sample', pt.size = 0.5)
p3.rb <- FeatureScatter(sce.all, "nCount_RNA", "percent_ribo", group.by = 'sample', pt.size = 0.5)
p3.rb.mito <- FeatureScatter(sce.all, "percent_ribo", "percent_mito", group.by = 'sample', pt.size = 0.5)
...
#p3 <- p31 + p32 + p33 + p34
ggsave(filename="Scatterplot.pdf", p3, width = 12, height = 8, dpi = 600)

##按照原文章处理方式
#Doublets were removed by removing cells whose nFeature < 0.22*nCount – 170.
sce.all.filt <- subset(sce.all, subset = nFeature_RNA >= (0.22*nCount_RNA-170))
dim(sce.all)#20791 41604
dim(sce.all.filt)#20791 41424

#过滤线粒体
sce.all.list <- SplitObject(sce.all, split.by = "sample")
...
selected_c <- WhichCells(sce.all, expression = nFeature_RNA > 600)#600
...
length(selected_hb)#32354
length(selected_ribo)#32824
length(selected_mito)#21553

sce.all.filt <- subset(sce.all.filt, cells = selected_mito)
dim(sce.all.filt)#18628 21553
...
#检测doublets 
# 筛选高变基因
sce.all.filt <- FindVariableFeatures(sce.all.filt, verbose = F)
sce.all.filt <- ScaleData(sce.all.filt, 
                          vars.to.regress = c("nFeature_RNA", "percent_mito"))
#降维
sce.all.filt <- RunPCA(sce.all.filt, npcs = 20)#提取20个主成分
...
library(dplyr)
rm(list = ls())
setwd("C:/Users/maihuanzhuo/Desktop/scRNA/scRNA-HIV/1-QC-7.22")
...
rm(list=ls())
sce.all <- readRDS( "C:/Users/maihuanzhuo/Desktop/scRNA/scRNA-HIV/1-QC-7.22/sce.all_int.rds")
...
ggsave("tsne_by_CCA_snn_res.0.8.pdf", p3, width = 8, height = 7, dpi = 600)


DefaultAssay(sce.all) <- "RNA"
markerGenes <- c(#"CD3D", "CD3E",# 定位T细胞
  #"TRAC",# 定位T细胞
  "IL7R", # CD4 T cells
  #"GZMA", # NK T /效应T
  "NKG7", # NK T cells
...
p1 <- VlnPlot(sce.all, features = markerGenes, pt.size = 0)
ggsave("VlnPlot_markerGenes.pdf", p1, width = 15, height = 10, dpi = 600)
...
ggsave("FeaturePlot_markerGenes.pdf", p3, width = 15, height = 10, dpi = 300)
#识别每个cluster的marker
...
control_samples <- grep("GSM4775588|pbmc3k", all_samples, value = TRUE)
...
conserved_markers <- FindConservedMarkers(sce.all, ident.1 = 2, #此参数表示一次仅评估一个cluster
...
celltype[celltype$ClusterID %in% c(0,2),2]='Naive CD4+ T'
celltype[celltype$ClusterID %in% c(1),2]='Memory CD4+ T'
celltype[celltype$ClusterID %in% c(3,4),2]='CD8+ T'
celltype[celltype$ClusterID %in% c(5,6),2]='B'
celltype[celltype$ClusterID %in% c(7,11),2]='NK'
celltype[celltype$ClusterID %in% c(8),2]='CD14+ Mono'
...
p4_split <- DimPlot(sce.all,reduction = "tsne", label=T, split.by ='group',pt.size = 1)
...
ggsave(filename="split_marker.pdf", p4_split, width = 12, height = 8, dpi = 600) 
...
rm(list = ls())
setwd("C:/Users/maihuanzhuo/Desktop/scRNA/scRNA-HIV/1-QC-7.20")
