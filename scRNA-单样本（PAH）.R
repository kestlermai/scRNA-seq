...
#缺少barcode信息对应样本，而且V2，V3处理样本的方式不了解，只能通过cellranger重新跑上游
setwd("C:/Users/maihuanzhuo/Desktop/scRNA/scRNA-PAH")
#读取数据
sce.data <- Read10X(data.dir = "./raw")
sce <- CreateSeuratObject(counts = sce.data, project = "GSE233189", min.cells = 3, min.features = 200)
...
fivenum(sce@meta.data$percent_ribo)#最小值、下四分位数、中位数、上四分位数、最大值
#计算红血细胞基因比例
...
p1 <- VlnPlot(sce, group.by = "orig.ident", features = feats, pt.size = 0, ncol = 2) + 
  NoLegend()
ggsave(filename="Vlnplot1.pdf", p1, width = length(unique(sce$orig.ident))/3+5, height = 5, dpi = 600)
feats <- c("percent_mito", "percent_ribo", "percent_hb")
p2 <- VlnPlot(sce, group.by = "orig.ident", features = feats, pt.size = 0, ncol = 3, same.y.lims = T) + 
  scale_y_continuous(breaks=seq(0, 100, 5)) +
  NoLegend()
ggsave(filename="Vlnplot2.pdf", p2, width = length(unique(sce$orig.ident))/3+5, height = 5, dpi = 600)

p3.feature <- FeatureScatter(sce, "nCount_RNA", "nFeature_RNA", group.by = "orig.ident", pt.size = 0.5)
p3.mito <- FeatureScatter(sce, "nCount_RNA", "percent_mito", group.by = "orig.ident", pt.size = 0.5)
p3.rb <- FeatureScatter(sce, "nCount_RNA", "percent_ribo", group.by = "orig.ident", pt.size = 0.5)
p3.rb.mito <- FeatureScatter(sce, "percent_ribo", "percent_mito", group.by = "orig.ident", pt.size = 0.5)
...
selected_c <- WhichCells(sce, expression = nFeature_RNA > 600)#600
...
rm(list=ls())
sce.all <- readRDS( "C:/Users/maihuanzhuo/Desktop/scRNA/scRNA-HIV/1-QC-7.21/sce.all_int.rds")
